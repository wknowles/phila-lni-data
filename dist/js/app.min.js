var margin={top:20,right:20,bottom:70,left:40},width=600-margin.left-margin.right,height=300-margin.top-margin.bottom,x=d3.scaleBand().rangeRound([0,width],.05).padding(.1),y=d3.scaleLinear().range([height,0]),xAxis=d3.axisBottom().scale(x),yAxis=d3.axisLeft().scale(y).ticks(10),svgBar=d3.select("#bar-chart").append("svg").attr("width",width+margin.left+margin.right).attr("height",height+margin.top+margin.bottom).append("g").attr("transform","translate("+margin.left+","+margin.top+")");d3.csv("https://data.phila.gov/resource/st4m-d4h3.csv?$select=issuedate,expirationdate,censustract,fulladdress&$where=issuedate>%272016-01-01T00:00:00.000%27&licensestatus=Active&revenuecode=3202&$order=issuedate&$$app_token=8nwKL3zJBeBIdPKTZzwuKspfh",function(t,e){if(t)throw t;e.forEach(function(t){t.censustract=+t.censustract,t.issuedate=moment(t.issuedate),t.expirationdate=moment(t.expirationdate)});var a=d3.nest().key(function(t){return t.issuedate.month()}).rollup(function(t){return t.length}).entries(e);x.domain(a.map(function(t){return t.key})),y.domain([0,d3.max(a,function(t){return t.value})]),svgBar.append("g").attr("class","x axis").attr("transform","translate(0,"+height+")").call(xAxis).selectAll("text").style("text-anchor","end").attr("dx","-.8em").attr("dy","-.55em").attr("transform","rotate(-90)"),svgBar.append("g").attr("class","y axis").call(yAxis).append("text").attr("transform","rotate(-90)").attr("y",6).attr("dy",".71em").style("text-anchor","end").text("Value"),svgBar.selectAll("bar").data(a).enter().append("rect").style("fill","steelblue").attr("x",function(t){return x(t.key)}).attr("width",x.bandwidth()).attr("y",function(t){return y(t.value)}).attr("height",function(t){return height-y(t.value)})});
function ready(t,e,a){var r={},s={};a.forEach(function(t){r[+t.censustract]=+t.count_censustract,s[+t.count_censustract]=+t.censustract});var n="#fee0d2",o="#de2d26",c=d3.scaleLinear().domain(d3.extent(a,function(t){return+t.count_censustract})).range([n,o]);svg.append("g").attr("class","map").selectAll("path").data(topojson.feature(e,e.objects.Census_Tracts_2010).features).enter().append("path").attr("d",path).attr("id",function(t){return r[t.properties.NAME10]}).style("fill",function(t){return void 0===r[d3.format(".0f")(t.properties.NAME10)]?"#fcfcfc":c(r[d3.format(".0f")(t.properties.NAME10)])}).on("mouseover",function(t){d3.select(this).transition().duration(300).style("opacity",1),div.transition().duration(300).style("opacity",1),div.html("Census tract = "+d3.format(".0f")(t.properties.NAME10)+"<br> No. of licenses = "+r[d3.format(".0f")(t.properties.NAME10)])}).on("mouseout",function(){d3.select(this).transition().duration(300).style("opacity",1),div.transition().duration(300).style("opacity",1)});var d=60,i=180,p=d3.select("#chart").append("svg").attr("id","key").attr("class","key").attr("width",d).attr("height",i),u=p.append("defs").append("svg:linearGradient").attr("id","gradient").attr("x1","100%").attr("y1","0%").attr("x2","100%").attr("y2","100%").attr("spreadMethod","pad");u.append("stop").attr("offset","0%").attr("stop-color",o).attr("stop-opacity",1),u.append("stop").attr("offset","100%").attr("stop-color",n).attr("stop-opacity",1),p.append("rect").attr("width",d-40).attr("height",i-20).style("fill","url(#gradient)").attr("transform","translate(0,10)");var l=d3.scaleLinear().range([i-20,0]).domain(d3.extent(a,function(t){return+t.count_censustract})),f=d3.axisRight(l);p.append("g").attr("class","y axis").attr("transform","translate(24,10)").call(f).append("text").attr("transform","rotate(-90)").attr("y",30).attr("dy","1em").style("text-anchor","end").text("Number of")}var width=470,height=500,div=d3.select("#chart").append("div").attr("class","tooltip"),svg=d3.select("#chart").append("svg").attr("width",width).attr("height",height).attr("class","map"),projection=d3.geoMercator().center([-74.94,40.002]).scale(8e4),path=d3.geoPath().projection(projection);queue().defer(d3.json,"data/census_tracts.json").defer(d3.csv,"https://data.phila.gov/resource/st4m-d4h3.csv?$select=censustract,count(censustract)&$group=censustract&$order=censustract&$where=issuedate>%272016-01-01T00:00:00.000%27&licensestatus=Active&revenuecode=3202&$$app_token=8nwKL3zJBeBIdPKTZzwuKspfh").await(ready);
